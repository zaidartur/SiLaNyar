...
const kategoriTerpilih = computed(() =>
    props.kategoriList.find(k => k.id === form.value.kategori_id)
)
...
<template>
    ...
    <AdminLayout>
        <div class="p-6 max-w-4xl mx-auto bg-white rounded shadow">
            ...
            <div class="mb-4">
                <label class="font-semibold block mb-1">Pilih Kategori</label>
                <select v-model="form.kategori_id" class="w-full border rounded px-2 py-1"
            ...
            <div v-if="props.pengajuan.status_pengajuan === 'proses_validasi'" class="flex gap-4">
                <button @click="handleVerifikasi('diterima')" class="bg-green-600 text-white px-4 py-2 rounded">
                    ✅ Terima Pengajuan
                </button>
                <button @click="handleVerifikasi('ditolak')" class="bg-red-600 text-white px-4 py-2 rounded">
                    ❌ Tolak Pengajuan
                </button>
            </div>

            <div v-else>
                <p class="text-sm text-gray-500 italic">Pengajuan sudah diverifikasi</p>
            </div>
        </div>
    </AdminLayout>
</template>
