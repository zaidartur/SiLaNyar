    ...
    <AdminLayout>
        <div class="p-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full rounded-lg bg-white divide-y divide-gray-300">
                    <thead>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="(item, index) in props.jenis_cairan" :key="item.id" :class="[
                            index % 2 === 0 ? 'bg-white' : 'bg-gray-50',
                            'hover:bg-gray-100 transition-colors'
                        ]">
                            <td class="px-6 py-4 text-black text-left">{{ item.kode_jenis_cairan }}</td>
                            <td class="px-6 py-4 text-black text-left">{{ item.nama }}</td>
                            <td class="px-6 py-4 text-black text-left">{{ item.batas_minimum }}</td>
                            <td class="px-6 py-4 text-black text-left">{{ item.batas_maksimum }}</td>
                            <td class="px-6 py-4 text-left">
                                <div class="flex gap-2">
                                    <button @click="openEditModal(item)" class="text-yellow-500">
                                        ✏️
                                    </button>
                ...
                <DialogContent
                    ...
                    <div class="text-center">
                        <p class="font-bold text-gray-900">
                            HAPUS JENIS CAIRAN {{ deletingJenisCairan?.kode_jenis_cairan }}
                        </p>
