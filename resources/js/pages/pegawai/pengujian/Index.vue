        ...
        <div class="p-6">
            <div class="mb-6 flex items-center justify-between">
                ...
                <Link
                    ...
                    :href="route('pegawai.pengujian.create')"
            ...
            <div v-if="unscheduled_pengajuan && unscheduled_pengajuan.length > 0" class="mb-4">
                <div class="rounded border-l-4 border-orange-500 bg-orange-100 p-3 text-orange-700">
                    <div class="flex">
                        <div class="ml-3">
                            <p class="text-sm">
                                <strong>⚠️ Peringatan:</strong> Ada {{ unscheduled_pengajuan.length }} pengajuan yang belum dijadwalkan untuk
                                pengujian:
                            </p>
                            <ul class="mt-1 list-inside list-disc text-xs">
                                <li v-for="item in unscheduled_pengajuan" :key="item.id">{{ item.kode_pengajuan }} - {{ item.instansi?.nama }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div class="mb-6 flex items-end gap-4">
                ...
                <button @click="resetFilter" class="rounded bg-gray-500 px-3 py-1 text-white transition hover:bg-gray-600">Reset</button>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full overflow-hidden rounded-xl bg-white shadow">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in pengujian" :key="item.id" :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-200'">
                            ...
                            <td class="border-b px-4 py-3">
                                <div class="flex gap-2">
                                    <Link :href="route('pegawai.pengujian.detail', item.id)" class="text-blue-500 hover:text-blue-700" title="Lihat">
                                        <span>👁️</span>
                                    </Link>
                                    <Link :href="route('pegawai.pengujian.edit', item.id)" class="text-yellow-500 hover:text-yellow-700" title="Edit">
                                        <span>✏️</span>
                                    </Link>
                                    <button @click="openDeleteModal(item)" class="text-red-500 hover:text-red-700" type="button" title="Hapus">
                                        <span>🗑️</span>
                                    </button>
                                </div>
                            </td>
