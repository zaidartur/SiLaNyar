...
const filteredKategori = computed(() => {
    if (!search.value) return props.kategori || [];
    return (props.kategori || []).filter((item: Kategori) =>
        item.nama.toLowerCase().includes(search.value.toLowerCase()) ||
        item.kode_kategori.toLowerCase().includes(search.value.toLowerCase())
    );
});

const totalPages = computed(() =>
    Math.max(1, Math.ceil(filteredKategori.value.length / pageSize))
);

const paginatedKategori = computed(() => {
    const start = (currentPage.value - 1) * pageSize;
    return filteredKategori.value.slice(start, start + pageSize);
});
...
<template>
    <Head title="Master Kategori Uji Lingkungan" />

    <AdminLayout title="Master Data Kategori">
        <div class="space-y-6">
            <!-- Header Halaman & Aksi -->
                ...
                <div>
                    <div class="flex items-center gap-2.5">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                            {{ filteredKategori.length }} Data
                        </span>
                    </div>
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                        Daftar pengelompokan jenis sampel, sub-kategori, dan parameter baku mutu terkait
                    </p>
                </div>
            ...
            <v-card rounded="2xl" elevation="1" class="border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-emerald-900 text-white dark:bg-emerald-950 dark:text-emerald-200 border-b border-emerald-800 dark:border-emerald-900 text-xs font-bold uppercase tracking-wider">
                                ...
                                <th class="py-3.5 px-5">Sub Kategori</th>
                                <th class="py-3.5 px-5">Tarif Dasar</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800/80 text-xs">
                            <tr
                                v-for="item in paginatedKategori"
                                :key="item.id"
                            ...
                            >
                                ...
                                <td class="py-3.5 px-5 text-slate-600 dark:text-slate-300">
                                    <div v-if="item.subkategori && item.subkategori.length > 0" class="flex flex-wrap gap-1">
                                        <span
                                            v-for="sub in item.subkategori"
                                            :key="sub.id"
                                            class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[10px] font-medium border border-slate-200 dark:border-slate-700"
                                        >
                                            {{ sub.nama }}
                                        </span>
                                    </div>
                                ...
                                <td class="py-3.5 px-5 text-slate-600 dark:text-slate-300 max-w-xs">
                            ...
                            <tr v-if="paginatedKategori.length === 0">
                                <td colspan="6" class="text-center py-12 text-slate-400 dark:text-slate-500">
                                    <v-icon size="36" class="mb-2 text-slate-300 dark:text-slate-600">mdi-tag-outline</v-icon>
                                    <p class="font-medium text-xs">Tidak ada data kategori ditemukan.</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="p-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <span class="text-xs text-slate-500 dark:text-slate-400">
                        Menampilkan halaman <strong>{{ currentPage }}</strong> dari <strong>{{ totalPages }}</strong> (Total {{ filteredKategori.length }} kategori)
                    </span>
            ...
            <!-- Modal Konfirmasi Hapus Vuetify Modern -->
                ...
                <v-card rounded="2xl" class="p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-center space-y-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-xs text-slate-600 dark:text-slate-300">
                            ...
                            <strong class="text-slate-900 dark:text-slate-100">{{ deletingKategori?.nama }}</strong>
                            ({{ deletingKategori?.kode_kategori }})? Tindakan ini dapat mempengaruhi sub-kategori terkait.
                        </p>
                    </div>
