<script setup lang="ts">
import CustomerLayout from '@/layouts/customer/CustomerLayout.vue'
import { ref, watch } from 'vue'
import { useForm, usePage, Head } from '@inertiajs/vue3'

interface JenisCairan {
    id: number;
    nama: string;
    batas_minimum: number;
    batas_maksimum: number;
}
interface Parameter {
    id: number;
    nama_parameter: string;
}
interface SubKategori {
    id: number;
    nama: string;
    parameter: Parameter[];
}
interface Kategori {
    id: number;
    nama: string;
    parameter: Parameter[];
    subkategori: SubKategori[];
}
interface Instansi {
    id: number
    nama: string
}

const { props } = usePage()
const instansiList = props.instansi as Instansi[]
const jenisCairan = props.jenis_cairan as JenisCairan[]
const kategori = props.kategori as Kategori[]
const semuaParameter = props.parameter as Parameter[]

const step = ref(1)
const pengajuanBerhasil = ref(false)

const form = useForm({
    id_instansi: '',
    id_jenis_cairan: null,
    volume_sampel: null,
    metode_pengambilan: '',
    lokasi: '',
    waktu_pengambilan: null,
    id_kategori: null,
    parameter: [] as number[],
    keterangan: ''
})

// Otomatis lokasi jika "diantar"
watch(() => form.metode_pengambilan, (val) => {
    if (val === 'diantar') {
        form.lokasi = 'Jl. Lawu No.204, Tegalasri, Bejen, Kec. Karanganyar, Kabupaten Karanganyar, Jawa Tengah 57716 (DLH Kabupaten Karanganyar)';
    } else {
        form.lokasi = '';
    }
    form.parameter = [];
    form.id_kategori = null;
})

function parameterIsInKategori(id: number): boolean {
    const kat = kategori.find(k => k.id === form.id_kategori);
    if (!kat) return true;
    const allowedParamIds = kat.subkategori.length > 0
        ? kat.subkategori.flatMap(s => s.parameter.map(p => p.id))
        : kat.parameter.map(p => p.id);
    return allowedParamIds.includes(id);
}

watch(() => form.id_kategori, (kategoriId) => {
    const kat = kategori.find(k => k.id === kategoriId);
    if (!kat) return;
    const allowedParamIds = kat.subkategori.length > 0
        ? kat.subkategori.flatMap(s => s.parameter.map(p => p.id))
        : kat.parameter.map(p => p.id);
    form.parameter = [...new Set(allowedParamIds)];
})

function nextStep() {
    if (step.value < 4) step.value += 1
}
function prevStep() {
    if (step.value > 1) step.value -= 1
}
function submit() {
    form.post(route('customer.pengajuan.store'), {
        onSuccess: () => {
            pengajuanBerhasil.value = true
            step.value = 3
        },
        onError: (errors) => {
            console.error(errors);
        }
    });
}
function getNamaJenisCairan() {
    return jenisCairan.find(j => j.id === form.id_jenis_cairan)?.nama || '-'
}
function getNamaInstansi() {
    return instansiList.find(i => i.id == form.id_instansi)?.nama || '-'
}
function getNamaKategori() {
    return kategori.find(k => k.id === form.id_kategori)?.nama || '-'
}
function getNamaParameter() {
    return semuaParameter.filter(p => form.parameter.includes(p.id)).map(p => p.nama_parameter)
}
</script>

<template>

    <Head title="Pengajuan Sampel" />
    <CustomerLayout>
        <!-- Stepper 4 langkah -->
        <ol class="flex items-center w-full mb-8">
            <!-- Step 1 -->
            <li class="flex items-center w-full after:content-[''] after:w-full after:h-1 after:border-b after:border-4 after:inline-block after:border-customLightGreen"
                :class="step > 1 ? 'text-customDarkGreen' : 'text-gray-400'">
                <div :class="[
                    'flex items-center justify-center w-10 h-10 rounded-full shrink-0',
                    step === 1 ? 'bg-customDarkGreen text-white' : step > 1 ? 'bg-customDarkGreen text-white' : 'bg-gray-100 text-gray-400'
                ]">
                    <span class="text-2xl font-bold">1</span>
                </div>
            </li>
            <!-- Step 2 -->
            <li class="flex items-center w-full after:content-[''] after:w-full after:h-1 after:border-b after:border-4 after:inline-block after:border-gray-100"
                :class="step > 2 ? 'text-customDarkGreen' : 'text-gray-400'">
                <div :class="[
                    'flex items-center justify-center w-10 h-10 rounded-full shrink-0',
                    step === 2 ? 'bg-customDarkGreen text-white' : step > 2 ? 'bg-customDarkGreen text-white' : 'bg-gray-100 text-gray-400'
                ]">
                    <span class="text-2xl font-bold">2</span>
                </div>
            </li>
            <!-- Step 3 -->
            <li class="flex items-center w-full after:content-[''] after:w-full after:h-1 after:border-b after:border-4 after:inline-block after:border-gray-100"
                :class="step > 3 ? 'text-customDarkGreen' : 'text-gray-400'">
                <div :class="[
                    'flex items-center justify-center w-10 h-10 rounded-full shrink-0',
                    step === 3 ? 'bg-customDarkGreen text-white' : step > 3 ? 'bg-blue-100 text-customDarkGreen' : 'bg-gray-100 text-gray-400'
                ]">
                    <span class="text-2xl font-bold">3</span>
                </div>
            </li>
            <!-- Step 4 -->
            <li class="flex items-center w-full">
                <div :class="[
                    'flex items-center justify-center w-10 h-10 rounded-full shrink-0',
                    step === 4 ? 'bg-customDarkGreen text-white' : step > 4 ? 'bg-blue-100 text-customDarkGreen' : 'bg-gray-100 text-gray-400'
                ]">
                    <span class="text-2xl font-bold">4</span>
                </div>
            </li>
        </ol>

        <!-- Step 1: Jenis Cairan, Volume, Instansi, Metode Pengambilan -->
        <form v-if="step === 1" @submit.prevent="nextStep" class="space-y-4">
            <div class="rounded-lg border border-gray-300 bg-gray-100 p-6 shadow-sm">
                <h3 class="text-lg font-bold text-customDarkGreen flex items-center gap-2 mb-2">
                    <svg width="33" height="30" viewBox="0 0 33 30" fill="none" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="33" height="30" fill="url(#pattern0_1392_980)" />
                        <defs>
                            <pattern id="pattern0_1392_980" patternContentUnits="objectBoundingBox" width="1"
                                height="1">
                                <use xlink:href="#image0_1392_980"
                                    transform="matrix(0.00909091 0 0 0.01 0.0454545 0)" />
                            </pattern>
                            <image id="image0_1392_980" width="100" height="100" preserveAspectRatio="none"
                                xlink:href="data:image/png;base64," />
                        </defs>
                    </svg>
                    Informasi Dasar Sample
                </h3>
                <label class="block mb-2 font-semibold text-gray-600">Jenis Cairan</label>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <div v-for="jenis in jenisCairan" :key="jenis.id" @click="form.id_jenis_cairan = jenis.id" :class="[
                        'cursor-pointer border rounded-2xl p-4 transition-all',
                        form.id_jenis_cairan === jenis.id
                            ? 'bg-customDarkGreen text-white border-customDarkGreen'
                            : 'bg-white text-customDarkGreen border-gray-300 hover:bg-customLightGreen'
                    ]">
                        <div :class="[
                            'font-bold text-2xl mb-1',
                            form.id_jenis_cairan === jenis.id ? 'text-white' : 'text-customDarkGreen'
                        ]">
                            {{ jenis.nama }}
                        </div>
                        <div :class="form.id_jenis_cairan === jenis.id ? 'text-white' : 'text-gray-500'"
                            class="text-xs">
                            Batas Minimum: {{ jenis.batas_minimum }}
                        </div>
                        <div :class="form.id_jenis_cairan === jenis.id ? 'text-white' : 'text-gray-500'"
                            class="text-xs">
                            Batas Maksimum: {{ jenis.batas_maksimum }}
                        </div>
                        <div v-if="form.id_jenis_cairan === jenis.id" class="mt-2 text-xs text-blue-400 font-semibold">
                            Dipilih
                        </div>
                    </div>
                </div>
                <!-- Hidden select for validation fallback -->
                <select v-model="form.id_jenis_cairan" class="hidden" required>
                    <option value="">Pilih Jenis Cairan</option>
                    <option v-for="jenis in jenisCairan" :key="jenis.id" :value="jenis.id">{{ jenis.nama }}</option>
                </select>
            </div>
            <div class="rounded-lg border border-gray-300 bg-gray-100 p-6 shadow-sm">
                <div class="grid grid-cols-1 gap-4">
                    <h3 class="text-lg font-bold text-customDarkGreen flex items-center gap-2">
                        <svg width="35" height="26" viewBox="0 0 35 26" fill="none" xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink">
                            <rect width="35" height="26" fill="url(#pattern0_1393_1239)" />
                            <defs>
                                <pattern id="pattern0_1393_1239" patternContentUnits="objectBoundingBox" width="1"
                                    height="1">
                                    <use xlink:href="#image0_1393_1239"
                                        transform="matrix(0.00742857 0 0 0.01 0.128571 0)" />
                                </pattern>
                                <image id="image0_1393_1239" width="100" height="100" preserveAspectRatio="none"
                                    xlink:href="data:image/png;base64," />
                            </defs>
                        </svg>
                        Informasi Volume dan Pengambilan
                    </h3>
                    <!-- Volume -->
                    <div>
                        <label class="block mb-1 font-semibold">Volume/Berat Sampel</label>
                        <div class="relative">
                            <!-- SVG Icon -->
                            <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg width="21" height="21" viewBox="0 0 21 21" fill="none"
                                    xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <rect width="21" height="21" fill="url(#pattern0_1393_1242)" />
                                    <defs>
                                        <pattern id="pattern0_1393_1242" patternContentUnits="objectBoundingBox"
                                            width="1" height="1">
                                            <use xlink:href="#image0_1393_1242" transform="scale(0.01)" />
                                        </pattern>
                                        <image id="image0_1393_1242" width="100" height="100" preserveAspectRatio="none"
                                            xlink:href="data:image/png;base64," />
                                    </defs>
                                </svg>
                            </span>
                            <input type="number" v-model.number="form.volume_sampel"
                                class="w-full border rounded px-3 py-2 pl-10" placeholder="Min : 2,5 ml" required />
                        </div>
                    </div>
                    <!-- Instansi -->
                    <div>
                        <label class="block mb-1 font-semibold">Instansi</label>
                        <select v-model="form.id_instansi" class="w-full border rounded px-3 py-2" required>
                            <option value="">Pilih Instansi</option>
                            <option v-for="ins in instansiList" :key="ins.id" :value="ins.id">{{ ins.nama }}</option>
                        </select>
                    </div>
                    <!-- Metode Pengambilan -->
                    <div>
                        <label class="block mb-1 font-semibold">Metode Pengambilan</label>
                        <div class="flex gap-2">
                            <button type="button" :class="[
                                'flex-1 px-3 py-2 rounded border font-semibold transition',
                                form.metode_pengambilan === 'diantar'
                                    ? 'bg-customDarkGreen text-white border-customdarkbg-customDarkGreen'
                                    : 'bg-white text-customDarkGreen border-gray-300 hover:bg-customLightGreen'
                            ]" @click="form.metode_pengambilan = 'diantar'">
                                Diantar
                            </button>
                            <button type="button" :class="[
                                'flex-1 px-3 py-2 rounded border font-semibold transition',
                                form.metode_pengambilan === 'diambil'
                                    ? 'bg-customDarkGreen text-white border-customDarkGreen'
                                    : 'bg-white text-customDarkGreen border-gray-300 hover:bg-customLightGreen'
                            ]" @click="form.metode_pengambilan = 'diambil'">
                                Diambil
                            </button>
                        </div>
                    </div>
                    <!-- Lokasi Pengambilan (jika diambil) -->
                    <div v-if="form.metode_pengambilan === 'diambil'" class="md:col-span-2 lg:col-span-1">
                        <label class="block mb-1 font-semibold">Lokasi Pengambilan</label>
                        <input type="text" v-model="form.lokasi" class="w-full border rounded px-3 py-2" required />
                    </div>
                    <!-- Waktu Pengambilan (jika diantar) -->
                    <div v-if="form.metode_pengambilan === 'diantar'" class="md:col-span-2 lg:col-span-1">
                        <label class="block mb-1 font-semibold">Waktu Pengambilan</label>
                        <input type="date" v-model="form.waktu_pengambilan" class="w-full border rounded px-3 py-2"
                            required />
                    </div>
                </div>
            </div>
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded">Lanjut</button>
        </form>

        <!-- Step 2: Kategori & Parameter -->
        <form v-else-if="step === 2" @submit.prevent="nextStep" class="space-y-6">
            <div class="rounded-lg border border-gray-300 bg-gray-100 p-6 shadow-sm space-y-4">
                <!-- Kategori -->
                <div>
                    <label class="text-xl text-customDarkGreen mb-1 font-semibold flex items-center gap-2">
                        <!-- SVG Icon -->
                        <svg width="33" height="30" viewBox="0 0 33 30" fill="none" xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink">
                            <rect width="33" height="30" fill="url(#pattern0_1392_980)" />
                            <defs>
                                <pattern id="pattern0_1392_980" patternContentUnits="objectBoundingBox" width="1"
                                    height="1">
                                    <use xlink:href="#image0_1392_980"
                                        transform="matrix(0.00909091 0 0 0.01 0.0454545 0)" />
                                </pattern>
                                <image id="image0_1392_980" width="100" height="100" preserveAspectRatio="none"
                                    xlink:href="data:image/png;base64," />
                            </defs>
                        </svg>
                        Pilih Kategori Pengujian
                    </label>
                    <select v-model="form.id_kategori" class="w-full border rounded px-3 py-2" required>
                        <option value="">Pilih Kategori</option>
                        <option v-for="kat in kategori" :key="kat.id" :value="kat.id">{{ kat.nama }}</option>
                    </select>
                </div>
                <!-- Parameter -->
                <div>
                    <label class="text-xl text-customDarkGreen block mb-1 font-semibold">Pilih Parameter</label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                        <div v-for="param in semuaParameter" :key="param.id"
                            class="flex items-center p-2 rounded hover:bg-blue-50 transition">
                            <input type="checkbox" :value="param.id" v-model="form.parameter"
                                :disabled="form.id_kategori && !parameterIsInKategori(param.id)" class="mr-2" />
                            <span :class="{
                                'text-gray-400': form.id_kategori && !parameterIsInKategori(param.id)
                            }">
                                {{ param.nama_parameter }}
                                <span v-if="form.id_kategori && !parameterIsInKategori(param.id)">
                                    (tidak termasuk kategori)
                                </span>
                            </span>
                        </div>
                    </div>
                </div>
            </div>
            <!-- Ringkasan Pengujian dan Harga -->
            <div>
                <div
                    class="rounded-t-lg bg-green-700 text-white px-4 py-2 flex items-center gap-2 text-base font-semibold">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M7.5 20C8.32843 20 9 19.3284 9 18.5C9 17.6716 8.32843 17 7.5 17C6.67157 17 6 17.6716 6 18.5C6 19.3284 6.67157 20 7.5 20Z"
                            fill="white" />
                        <path
                            d="M16.5 20C17.3284 20 18 19.3284 18 18.5C18 17.6716 17.3284 17 16.5 17C15.6716 17 15 17.6716 15 18.5C15 19.3284 15.6716 20 16.5 20Z"
                            fill="white" />
                        <path d="M3 5H5L5.6 8M5.6 8L7 15H17L19 8H5.6Z" fill="white" />
                        <path d="M3 5H5L5.6 8M5.6 8L7 15H17L19 8H5.6Z" stroke="white" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                    Ringkasan Pengujian
                </div>
                <table
                    class="w-full border border-green-700 rounded-b-lg overflow-hidden border-separate border-spacing-0">
                    <thead>
                        <tr>
                            <th
                                class="text-center font-semibold bg-green-100 border-b border-green-700 px-4 py-3 w-1/3">
                                Nama Parameter
                            </th>
                            <th
                                class="text-center font-semibold bg-green-100 border-b border-green-700 px-4 py-3 w-1/3">
                                Harga
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="param in semuaParameter.filter(p => form.parameter.includes(p.id))" :key="param.id"
                            class="hover:bg-green-50 transition">
                            <td class="text-left px-4 py-3 border-b border-green-2000">
                                {{ param.nama_parameter }}
                            </td>
                            <td class="text-right px-4 py-3 border-b border-green-200">
                                Rp {{ param.harga?.toLocaleString('id-ID') || '0' }}
                            </td>
                        </tr>
                        <tr v-if="form.parameter.length === 0">
                            <td colspan="2" class="text-center text-gray-400 py-4 border-b border-green-200">Belum ada
                                parameter dipilih</td>
                        </tr>
                    </tbody>
                </table>
                <div
                    class="mt-2 bg-gray-100 border border-gray-300 rounded-lg px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <span class="text-sm text-gray-600">Jumlah Parameter:<br><b>{{ form.parameter.length }}
                            item</b></span>
                    <span class="text-lg font-semibold text-gray-700 mt-2 sm:mt-0">
                        Biaya:<br>
                        <span class="text-green-700 text-2xl font-bold">
                            Rp {{
                            semuaParameter
                            .filter(p => form.parameter.includes(p.id))
                            .reduce((sum, p) => sum + (p.harga || 0), 0)
                            .toLocaleString('id-ID')
                            }}
                        </span>
                    </span>
                </div>
            </div>
            <div class="flex gap-2">
                <button type="button" @click="prevStep" class="px-4 py-2 bg-gray-300 rounded">Kembali</button>
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded">Lanjut</button>
            </div>
        </form>

        <!-- Step 3: Periksa & Serahkan -->
        <form v-else-if="step === 3" @submit.prevent="submit" class="space-y-4">
            <div class="pt-4">
                <h2 class="font-bold mb-4 text-lg text-customDarkGreen flex items-center gap-2">
                    <svg width="28" height="28" fill="none" viewBox="0 0 24 24">
                        <path d="M12 2a10 10 0 100 20 10 10 0 000-20zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z" fill="#059669" />
                    </svg>
                    Periksa Data Pengajuan
                </h2>
                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                    <table class="w-full mb-4">
                        <tbody>
                            <tr>
                                <td class="py-2 pr-4 text-gray-600 font-semibold w-1/3">Jenis Cairan</td>
                                <td class="py-2 text-gray-800">{{ getNamaJenisCairan() }}</td>
                            </tr>
                            <tr>
                                <td class="py-2 pr-4 text-gray-600 font-semibold">Volume/Berat Sampel</td>
                                <td class="py-2 text-gray-800">{{ form.volume_sampel }}</td>
                            </tr>
                            <tr>
                                <td class="py-2 pr-4 text-gray-600 font-semibold">Instansi</td>
                                <td class="py-2 text-gray-800">{{ getNamaInstansi() }}</td>
                            </tr>
                            <tr>
                                <td class="py-2 pr-4 text-gray-600 font-semibold">Metode Pengambilan</td>
                                <td class="py-2 text-gray-800">{{ form.metode_pengambilan }}</td>
                            </tr>
                            <tr v-if="form.metode_pengambilan === 'diambil'">
                                <td class="py-2 pr-4 text-gray-600 font-semibold">Lokasi Pengambilan</td>
                                <td class="py-2 text-gray-800">{{ form.lokasi }}</td>
                            </tr>
                            <tr v-if="form.metode_pengambilan === 'diantar'">
                                <td class="py-2 pr-4 text-gray-600 font-semibold">Waktu Pengambilan</td>
                                <td class="py-2 text-gray-800">{{ form.waktu_pengambilan }}</td>
                            </tr>
                            <tr>
                                <td class="py-2 pr-4 text-gray-600 font-semibold">Kategori</td>
                                <td class="py-2 text-gray-800">{{ getNamaKategori() }}</td>
                            </tr>
                            <tr>
                                <td class="py-2 pr-4 text-gray-600 font-semibold align-top">Parameter</td>
                                <td class="py-2 text-gray-800">
                                    <ul class="list-disc ml-5">
                                        <li v-for="param in semuaParameter.filter(p => form.parameter.includes(p.id))"
                                            :key="param.id">
                                            {{ param.nama_parameter }}
                                            <span class="text-gray-500 ml-2">Rp {{ param.harga?.toLocaleString('id-ID')
                                                || '0' }}</span>
                                        </li>
                                    </ul>
                                    <div v-if="form.parameter.length === 0" class="text-gray-400">Belum ada parameter
                                        dipilih</div>
                                </td>
                            </tr>
                            <tr v-if="form.keterangan">
                                <td class="py-2 pr-4 text-gray-600 font-semibold">Keterangan</td>
                                <td class="py-2 text-gray-800">{{ form.keterangan }}</td>
                            </tr>
                        </tbody>
                    </table>
                    <div
                        class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
                        <span class="text-sm text-gray-600">Jumlah Parameter: <b>{{ form.parameter.length }}
                                item</b></span>
                        <span class="text-lg font-semibold text-gray-700 mt-2 sm:mt-0">
                            Total Biaya:
                            <span class="text-green-700 text-2xl font-bold ml-2">
                                Rp {{
                                    semuaParameter
                                        .filter(p => form.parameter.includes(p.id))
                                        .reduce((sum, p) => sum + (p.harga || 0), 0)
                                        .toLocaleString('id-ID')
                                }}
                            </span>
                        </span>
                    </div>
                </div>
            </div>
            <div class="flex gap-2">
                <button type="button" @click="prevStep" class="px-4 py-2 bg-gray-300 rounded">Kembali</button>
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded">Kirim Pengajuan</button>
            </div>
        </form>

        <!-- Step 4: Pembayaran -->
        <div v-else-if="step === 4" class="space-y-4">
            <div v-if="pengajuanBerhasil" class="p-4 bg-green-100 border border-green-300 rounded">
                <b>Pengajuan berhasil dikirim!</b>
            </div>
            <div class="p-4 bg-blue-50 border border-blue-200 rounded">
                <h2 class="font-bold mb-2">Instruksi Pembayaran</h2>
                <p>Silakan lakukan pembayaran ke rekening berikut:</p>
                <ul class="mb-2">
                    <li><b>Bank:</b> BRI</li>
                    <li><b>No. Rekening:</b> 1234-5678-9012-3456</li>
                    <li><b>Atas Nama:</b> DLH Karanganyar</li>
                </ul>
                <p>Setelah pembayaran, silakan konfirmasi ke admin.</p>
            </div>
        </div>
    </CustomerLayout>
</template>