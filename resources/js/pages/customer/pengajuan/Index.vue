...
import { Check, CheckCircle, Circle, Dot } from 'lucide-vue-next';
import { h, ref } from 'vue';
import * as z from 'zod';

const formSchema = [
    // Step 1: Detail Sample
    z
        .object({
            jenisCairan: z.string().min(1, 'Pilih jenis cairan'),
            volume: z.number({ invalid_type_error: 'Volume harus diisi' }).min(2.5, 'Volume minimal 2.5 L'),
            instansi: z.string().min(1, 'Pilih instansi'),
            metodePengambilan: z.string().min(1, 'Pilih metode pengambilan'),
            tanggalPengantaran: z.string().optional(),
            lokasiPengambilan: z.string().optional(),
        })
        .refine(
            (data) => {
                if (data.metodePengambilan === 'diantar') {
                    return !!data.tanggalPengantaran;
                }
                if (data.metodePengambilan === 'dijemput') {
                    return !!data.lokasiPengambilan;
                }
                return true;
            },
            {
                message: 'Tanggal pengantaran/lokasi pengambilan harus diisi',
                path: ['metodePengambilan'],
            },
        ),

    // Step 2: Parameter Pengujian
    z
        .object({
            parameterFisika: z.object({
                suhu: z.boolean().optional(),
                ph: z.boolean().optional(),
                dhl: z.boolean().optional(),
                tss: z.boolean().optional(),
                tds: z.boolean().optional(),
                kekeruhan: z.boolean().optional(),
                salinitas: z.boolean().optional(),
                warna: z.boolean().optional(),
            }),
            parameterKimia: z.object({
                cod: z.boolean().optional(),
                bod: z.boolean().optional(),
                phosphate: z.boolean().optional(),
                nitrat: z.boolean().optional(),
                nitrit: z.boolean().optional(),
                mbas: z.boolean().optional(),
                minyakLemak: z.boolean().optional(),
                fenol: z.boolean().optional(),
                nh3n: z.boolean().optional(),
            }),
        })
        .refine(
            (data) => {
                const anyFisikaSelected = Object.values(data.parameterFisika).some(Boolean);
                const anyKimiaSelected = Object.values(data.parameterKimia).some(Boolean);
                return anyFisikaSelected || anyKimiaSelected;
            },
            {
                message: 'Pilih minimal satu parameter pengujian',
                path: ['parameterFisika'],
            },
        ),

    // Step 3: Review dan Serahkan
    z.object({
        konfirmasiData: z.boolean().refine((val) => val === true, {
            message: 'Anda harus mengkonfirmasi kebenaran data',
        }),
        persetujuanLayanan: z.boolean().refine((val) => val === true, {
            message: 'Anda harus menyetujui syarat layanan',
        }),
    }),

    // Step 4: Pembayaran
    z.object({
        metodePembayaran: z.enum(['transfer', 'tunai'], {
...
function onSubmit(values: z.infer<(typeof formSchema)[number]>) {
    toast({
...
const handleSubmit = async (values, { resetForm }) => {
    ...
    console.log('handleSubmit called', stepIndex.value);
};
</script>
                        ...
                        <div>
                                        ...
                                        <StepperTrigger as-child>
                                            <Button
                                                :class="[
                                                    ...
                                                    state === 'active'
                                                        ? 'bg-customDarkGreen text-white ring-2 ring-customDarkGreen ring-offset-2'
                                                        : '',
                            ...
                            <div class="mt-4 flex flex-col gap-4">
                                <template v-if="stepIndex === 1">
                                    <div class="space-y-6">
                                                        ...
                                                        >
                                                            <div class="flex flex-col items-center gap-2">
                                                                <svg
                                                                    ...
                                                                    xmlns:xlink="http://www.w3.org/1999/xlink"
                                                                >
                                                                    <ellipse cx="29.5" cy="28" rx="29.5" ry="28" fill="#F5F5F5" />
                                                                    <rect
                                                                        x="9"
                                                                        y="12"
                                                                        width="42"
                                                                        height="33"
                                                                        fill="url(#pattern0_1433_1546)"
                                                                        fill-opacity="0.78"
                                                                    />
                                                                    <defs>
                                                                        <pattern
                                                                            id="pattern0_1433_1546"
                                                                            patternContentUnits="objectBoundingBox"
                                                                            width="1"
                                                                            height="1"
                                                                        >
                                                                            <use
                                                                                xlink:href="#image0_1433_1546"
                                                                ...
                                                                >
                                                                    <ellipse cx="29.5" cy="28" rx="29.5" ry="28" fill="#F5F5F5" />
                                                                    <rect x="5" y="7" width="50" height="42" fill="url(#pattern0_1433_1547)" />
                                                                    <defs>
                                                                        <pattern
                                                                            id="pattern0_1433_1547"
                                                                            patternContentUnits="objectBoundingBox"
                                                                            width="1"
                                                                            height="1"
                                                                        >
                                                                            <use
                                                                                xlink:href="#image0_1433_1547"
                                                                ...
                                                                >
                                                                    <ellipse cx="29.5" cy="28" rx="29.5" ry="28" fill="#F5F5F5" />
                                                                    <rect x="3" y="9" width="54" height="37" fill="url(#pattern0_1433_1548)" />
                                                                    <defs>
                                                                        <pattern
                                                                            id="pattern0_1433_1548"
                                                                            patternContentUnits="objectBoundingBox"
                                                                            width="1"
                                                                            height="1"
                                                                        >
                                                                            <use
                                                                                xlink:href="#image0_1433_1548"
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <FormField
                                                v-if="values.metodePengambilan === 'diantar'"
                                            ...
                                            <FormField
                                                v-if="values.metodePengambilan === 'dijemput'"
                                                v-slot="{ componentField }"
                                    ...
                                    <div class="space-y-6">
                                        ...
                                        <div class="space-y-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <h3 class="text-lg font-medium text-customDarkGreen">Pilih Kategori Pengujian</h3>
                                            </div>
                                            <Select>
                                                <SelectTrigger>
                                ...
                                <template v-if="stepIndex === 3">
                                    <div class="space-y-4">
                                        <FormField v-slot="{ componentField }" name="konfirmasiData">
                                            <FormItem>
                                                <FormLabel>
                                                    <span class="font-semibold">Konfirmasi Data</span>
                                                </FormLabel>
                                                <FormControl>
                                                    <Checkbox v-bind="componentField" />
                                                </FormControl>
                                                <span class="text-xs text-gray-500">Saya telah memeriksa dan data sudah benar.</span>
                                                <FormMessage />
                                            </FormItem>
                                        </FormField>
                                        <FormField v-slot="{ componentField }" name="persetujuanLayanan">
                                            <FormItem>
                                                <FormLabel>
                                                    <span class="font-semibold">Persetujuan Layanan</span>
                                                </FormLabel>
                                                <FormControl>
                                                    <Checkbox v-bind="componentField" />
                                                </FormControl>
                                                <span class="text-xs text-gray-500">Saya menyetujui syarat dan ketentuan layanan.</span>
                                                <FormMessage />
                                            </FormItem>
                                        </FormField>
                                    </div>
                                ...
                                <template v-if="stepIndex === 4">
                                    <div class="space-y-4">
                                        <FormField v-slot="{ componentField }" name="metodePembayaran">
                                            <FormItem>
                                        ...
                                        <FormField v-slot="{ componentField }" name="buktiPembayaran">
                                            <FormItem>
                                                <FormLabel>Bukti Pembayaran (Opsional, max 2MB)</FormLabel>
                                                <FormControl>
                            ...
                            <div class="boerder-t mt-8 flex items-center justify-between pt-6">
                                <Button :disabled="isPrevDisabled" variant="outline" size="sm" @click="prevStep"> Back </Button>
                                <div class="flex items-center gap-3">
                                    <Button v-if="stepIndex !== steps.length" type="submit" size="sm" :disabled="!meta.valid"> Next </Button>
                                    <Button v-if="stepIndex === steps.length" type="submit" size="sm" :disabled="!meta.valid"> Submit </Button>
                                </div>
                            </div>
                        </div>
                    </Stepper>
